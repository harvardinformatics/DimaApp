...
imputeNormalizeTransform <- function(set, method='MLE') {
    # set: a MSnSet object
    # method: 
    iset <- impute(set, method)
    nset <- normalize(iset, 'quantiles')
...
x227.df <- read.csv('../MeltonProteomics/RawData/160621LDSSAM01867_227-2T6plx-FTK1-60_NOT_Normalized_PD22.csv')
...
# remove Accession 'uc', don't know what this is.
dat243.df <- dat243.df[!dat243.df$Accession == 'uc',]
...
colnames(dat243.df) <- paste('S243', paste('S', seq(0,5), 'c', sep=''), sep='_')
...
adat227.df <- namerows(dat227.df, col.name='features') # 9858
adat238.df <- namerows(dat238.df, col.name='features') # 9055
adat239.df <- namerows(dat239.df, col.name='features') # 8732
adat243.df <- namerows(dat243.df, col.name='features') # 9895
